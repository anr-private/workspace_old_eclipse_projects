...
public class DialView extends View implements DialModel.Listener, OnGestureListener {
	...
	*/
	private static Paint createDefaultPaint() {
		...
		paint.setFilterBitmap(true); // turns on smoothing (if/when bitmap is scaled, etc.)
		return paint;
	}
	...
	*/
	...
	private void init() {
		...
		/* @@@@ DISABLED SOUND initSoundPoolIfNecessary(getContext()); */
	...
	public final void setModel(DialModel model) {
		Log.d("DialView", "setModel model=" + model);
		if (this.model != null) {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (++redrawTimes >= 10) {
			Log.d(TAG, "Avg time = " + (redrawTime / ((float) redrawTimes)) + " ms");
	...
	public void onDialPositionChanged(DialModel sender, int nicksChanged) {
		...
		/* @@@@ DISABLED SOUND playSound(nicksChanged);   */
	}
	...
	*/
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		Log.d("DialView", "onRestore SETS THE MODEL");
		// @@@@ ANR MODEL FIX  setModel(DialModel.restore(bundle));
		this.model.restoreFromBundle(bundle);
	}
	...
	public boolean onScroll(MotionEvent eventA, MotionEvent eventB, float dx, float dy) {
		...
		Log.d("DialView", "onScroll dx=" + dx + " dy=" + dy);
		...
		if (! Float.isNaN(dragStartDeg)) {
			...
			if (! Float.isNaN(currentDeg)) {
				...
				if (nicks != 0) {
					...
					Log.d("DialView", "onScroll  POST RUNNABLE degs=" + currentDeg+ " nicks=" + nicks);
					handler.post(new Runnable() {
						//@Override
						public void run() {
							Log.d("DialView.RUNNABLE", "RUNNING nicks=" + nicks);
							model.rotate(nicks);								
						}
					});
				} 
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// redraw the contents of the DrawLayers
		regenerateLayers(w);
	}
